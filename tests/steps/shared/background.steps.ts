...
Given('the BlockKlout platform is available', async ({ request, page }) => {
  // This step works for both API and UI contexts
  if (request) {
    // API context - for demo purposes, we'll use a public API that actually exists
    // In real BlockKlout project, this would be the actual API endpoint
    ...
    // In real BlockKlout project, this would be the actual website
